# -*- coding: utf-8 -*-
"""
Created on Thu Nov 21 13:40:50 2024
...
class IGTDValDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if (idx <10):
            filename = "00000" + str(idx) + ".png"
        elif (idx < 100):
            filename = "0000" + str(idx) + ".png"
        else:
            filename = "000" + str(idx) + ".png"
        image_path = "../../Data/PictData/IGTD/Validation/" + filename
        image = decode_image(image_path).type(torch.FloatTensor)
        if self.transform:
            image = self.transform(image)
        return image
    
loaded_model = keras.saving.load_model("Saved_Models/UnbViTIGTD_model.keras")

validation_dataset = IGTDValDataset()
validation_data1 = []
validation_data2 = []
for i in range(475):
...
predictionForKaggle_df.to_csv("UnbIGTDVTPrediction.csv", index=False)
